...
class EdimdomaPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if item['description']:
            ...
            recipe_row['desc'] = description

        recipe_row['url'] = item['recipe_url']

        recipe_date = ''.join(item['date']).replace(u'Добавлено:','').strip()
        recipe_row['date'] = recipe_date
        recipe_row['rating'] = item['rating']
